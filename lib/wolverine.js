...
var getMessage = function getMessage(wolverineObj, style, fnArguments, levelName, levelPriority) {
  ...
  if(wolverineObj.level <= levelPriority) {
    ...
    if(typeof wolverineObj.options.printFileInfo !== 'undefined' && wolverineObj.options.printFileInfo) {
      var error = new Error();
      // var line = error.stack.split('\n').join('$$');
      var line = error.stack.split('\n')[3].split(':');
      var file = line[line.length - 3];
    ...
    if(args.length === 0) {
        ...
        message = time + fileInfo + level + args[0];
    }

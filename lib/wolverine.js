...
var Wolverine = function Wolverine(level, options) {
    ...
    var writingToTerminal = true;

    //Save the outputStream to use on "this._write()"
    if(typeof this.options.output !== 'undefined') {
        writingToTerminal = false;
        this.outputStream = this.options.output;

        if(typeof this.outputStream === 'string') {
            this.outputStream = fs.createWriteStream(this.options.output);
        }
    }
    else {
    ...
    //If it's not writing to the terminal,
    //should remove the colors
        ...
        if(wolverineObj.level <= levelObj.levelPriority) {
            ...
            switch(args.length) {
                ...
                case 2:
                    ...
                    break;
                default:
                    message = time + fileInfo + levelOnMessage + args[0] + ': ' + util.format.apply(this, args.slice(1));
            }

            if(typeof wolverineObj.options.silent === 'undefined' || !wolverineObj.options.silent) {
                this._write(levelObj.writer(message));
            }
        }
